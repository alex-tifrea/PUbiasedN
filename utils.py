...
class DataWrapper:
    def __init__(
        ...
        self.num_classes = (
            lib_data.get_num_classes(source_dataset, reindex_labels=True)
        ...
        if target_ood_ratio == 0.5 and self.target_id_size != self.target_ood_size:
            ...
            self.target_id_data = self.target_id_data.shuffle(target_size).take(
                final_size
            )
            self.target_ood_data = self.target_ood_data.shuffle(target_size).take(
                final_size
            )
